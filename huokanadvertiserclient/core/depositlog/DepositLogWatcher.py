...
class DepositLogWatcher:
    def __init__(self, config: Configuration, api_client: AuthenticatedClient) -> None:
        self._api_client = api_client
        self._config = config
        self._subject = Subject()
        self._watcher = SavedVariablesWatcher(config.wow_path, self._subject)
    ...
    def upload_log_from_file(self, file_path: Path):
        ...
        for log in deposit_logs:
            ...
            if guilds.parsed:
                ...
                # TODO ensure success and report error otherwise
            else:
                logging.info("failed")
